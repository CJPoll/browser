...
module Repositories
  # Hosts allowed to open popup windows.
  #
  ...
  class PopupExceptionRepository
    ...
    # @return [Boolean] True if the host may open popups
    def exists?(host)
      return false unless host

      !@db.get_first_value('SELECT id FROM popup_exceptions WHERE host = ?', [host]).nil?
    end

    # Grants popup permission to a host
    #
